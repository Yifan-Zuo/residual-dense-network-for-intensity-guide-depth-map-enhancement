...
with tf.variable_scope("gen_down3_inten"):
    guided_16xto8x_gen=gslb.inten_downsample_simple_unit(guided_8xto4x_gen, phase_train=phase_train)
...
with tf.variable_scope("gen_up4_dep"):
    dep_ten=gslb.LR_dep_fusion_simple_unit(dep_ten, guided_16xto8x_gen, phase_train=phase_train)
    dep_ten=gslb.LR_dep_upsampling_unit(dep_ten, phase_train=phase_train)
...
with tf.variable_scope("gen_up2_dep"):
    ...
    dep_ten=gslb.LR_dep_upsampling_unit(dep_ten, phase_train=phase_train)
with tf.variable_scope("gen_16x_last4_layers"):
...
# [HR_dy,HR_dx]=tf.image.image_gradients(HR_depth_batch_input)
# [gen_dy,gen_dx]=tf.image.image_gradients(gen_ten)
# HR_dy=HR_dy[:,0:height,:,:]
# gen_dy=gen_dy[:,0:height,:,:]
# HR_dx=HR_dx[:,:,0:width,:]
# gen_dx=gen_dx[:,:,0:width,:]
...
#loss=tf.reduce_mean(tf.abs(gen_ten-HR_depth_batch_input))+tf.reduce_mean(tf.abs(gen_dy-HR_dy)+tf.abs(gen_dx-HR_dx))
...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        ...
        saver_full.restore(sess, "/media/kenny/Data/trained_models/residual_dense_models/noisy/16x/full_model1/16x_ny_full_model.ckpt-44")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%3828==0:
                    save_path=saver_full.save(sess,"/media/kenny/Data/trained_models/residual_dense_models/noisy/16x/l1loss/full_model1/16x_ny_full_model.ckpt",global_step=model_ind)
                    print("Full Model saved in file: %s" % save_path)
                    model_ind=model_ind+1
            #save_path=saver.save(sess, "/home/yifan/eclipse4.6_workspace/guided_srgan_multi_skip_version/train/16x_pretrain_comp_gen/std5_version/reuse_part1/16x_comp_gen_model_part.ckpt",global_step=epo) 
            #print("Reusing Model saved in file: %s" % save_path)
            #save_path=saver_full.save(sess,"/media/yf/Data/yf/trained_models/guided_depth_up/noise_free/16x/full_model1/16x_nf_full_model.ckpt",global_step=epo)
            #print("Full Model saved in file: %s" % save_path)
