...
train_op_large = tf.train.AdamOptimizer(1e-5).minimize(loss)
...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        ...
        saver_full.restore(sess, "/media/kenny/Data/trained_models/residual_dense_models/noise_free/2x/full_model1/2x_nf_full_model.ckpt-59")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%3444==0:
                    save_path=saver_full.save(sess,"/media/kenny/Data/trained_models/residual_dense_models/noise_free/2x/full_model2/2x_nf_full_model.ckpt",global_step=model_ind)
                    print("Full Model saved in file: %s" % save_path)
                    model_ind=model_ind+1
            #save_path=saver.save(sess, "/home/yifan/eclipse4.6_workspace/guided_srgan_multi_skip_version/train/2x_pretrain_comp_gen/std5_version/modified_bn_code/reuse_part1/2x_comp_gen_model_part.ckpt",global_step=epo) 
            #print("Reusing Model saved in file: %s" % save_path)
            #save_path=saver_full.save(sess,"/media/yf/Data/yf/trained_models/guided_depth_up/noisy/2x/full_model1/2x_ny_full_model.ckpt",global_step=epo)
            #print("Full Model saved in file: %s" % save_path)
