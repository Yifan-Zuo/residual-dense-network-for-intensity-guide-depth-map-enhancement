...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        ...
        saver_full.restore(sess, "/media/kenny/Data/trained_models/residual_dense_models/noise_free/4x/full_model1/4x_nf_full_model.ckpt-47")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%3828==0:
                    save_path=saver_full.save(sess,"/media/kenny/Data/trained_models/residual_dense_models/noise_free/4x/l1loss/full_model1/4x_nf_full_model.ckpt",global_step=model_ind)
                    print("Full Model saved in file: %s" % save_path)
                    model_ind=model_ind+1
            #save_path=saver.save(sess, "/home/yifan/eclipse4.6_workspace/guided_srgan_multi_skip_version/train/4x_pretrain_comp_gen/std5_version/reuse_part1/4x_comp_gen_model_part.ckpt",global_step=epo) 
            #print("Reusing Model saved in file: %s" % save_path)
            #save_path=saver_full.save(sess,"/media/yf/Data/yf/trained_models/guided_depth_up/noise_free/4x/full_model2/4x_nf_full_model.ckpt",global_step=epo)
            #print("Full Model saved in file: %s" % save_path)
